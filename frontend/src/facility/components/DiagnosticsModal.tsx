...
import type { FacilityDiagnosticItem } from '../../services/facilitiesApi';
...
export const DiagnosticsModal: React.FC<DiagnosticsModalProps> = ({
  ...
  const filtered = diagnosticOrders.filter((item) => {
    const testName = item.testName || '';
    const patName = item.patient?.name || '';
    const docName = item.orderedByDoctor?.name || '';
    ...
    const isReviewed = ['REVIEWED', 'REPORT_UPLOADED', 'COMPLETED'].includes(item.status);
    const matchesStatus =
      statusFilter === 'ALL' ||
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto safe-area-pb">
      <div className="bg-white rounded-3xl max-w-4xl w-full p-5 sm:p-6 space-y-4 shadow-2xl border border-teal-200 animate-in fade-in zoom-in-95 duration-150 my-auto max-h-[90vh] flex flex-col">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-black text-slate-900 leading-tight">
                Diagnostic Orders & Lab Coordination
              </h3>
              <p className="text-xs text-slate-500">
                Facility pathology and diagnostic investigations tracked in real time
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search test name (CBC, etc.), patient, doctor..."
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-3 min-h-[240px]">
          {filtered.length === 0 ? (
            <div className="p-12 text-center text-slate-400 text-xs bg-slate-50 rounded-2xl border border-dashed border-slate-200">
              No diagnostic orders matching the criteria.
            </div>
          ) : (
            <div className="space-y-2.5">
              {filtered.map((item) => {
                const patientName = item.patient?.name || 'Assigned Patient';
                const doctorName = item.orderedByDoctor?.name || 'Attending Physician';
                const isReviewed = ['REVIEWED', 'REPORT_UPLOADED', 'COMPLETED'].includes(item.status);

                return (
                  <div
                    key={item.id}
                    className="p-3.5 sm:p-4 rounded-2xl bg-white border border-slate-200 hover:border-teal-200 transition shadow-2xs hover:shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                  >
                    <div className="space-y-1.5 min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[11px] text-slate-500 font-medium">
                          for <strong className="text-slate-800">{patientName}</strong>
                        </span>
                      ...
                      <div className="flex items-center gap-4 text-xs text-slate-500 flex-wrap">
                        <span className="flex items-center gap-1">
                          ...
                          <span>Ordered by: {doctorName}</span>
                        </span>
                        <span className="flex items-center gap-1 text-slate-400">
                          <Calendar className="w-3 h-3" />
                          <span>Ordered: {new Date(item.orderedAt).toLocaleDateString()}</span>
                        </span>
                      </div>

                      {item.resultSummary ? (
                        <div className="text-xs bg-emerald-50/60 p-2.5 rounded-xl border border-emerald-100 text-emerald-950">
                          <span className="font-bold text-emerald-800">Result Summary: </span>
                          <span>{item.resultSummary}</span>
                        </div>
                      ) : (
                        <p className="text-[11px] text-slate-400 italic">
                          Lab sample registered. Results awaiting lab review and doctor sign-off.
                        </p>
                      )}
                    </div>
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
          <span>Showing {filtered.length} of {diagnosticOrders.length} lab orders</span>
          <button
            type="button"
            onClick={onClose}
