...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-3 sm:top-5 z-50 w-full px-2 sm:px-4 lg:px-6 flex justify-center pointer-events-none transition-all duration-300">
      <div className="pointer-events-auto w-full max-w-[98%] sm:max-w-[96%] lg:max-w-[95%] xl:max-w-[96%] bg-white/95 backdrop-blur-md rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.07)] border border-slate-100/90 px-4 sm:px-6 lg:px-8 py-3 sm:py-3.5 lg:py-4 flex items-center justify-between gap-3 sm:gap-4 transition-all duration-300">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <button
            ...
            onClick={() => navigate('/')}
          >
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-orange-50 flex items-center justify-center p-1.5 shadow-xs transition group-hover:scale-105">
              <img src="/logo.svg" alt={t('logoAlt')} className="w-full h-full object-contain" />
            </div>
            <span className="font-extrabold text-slate-900 text-lg sm:text-xl tracking-tight brand-title">
        ...
        <div className="flex items-center gap-6 sm:gap-10 shrink-0">
          {/* Language Toggle */}
          <div className="language-toggle flex items-center bg-slate-100/80 p-1 rounded-full border border-slate-200/60" role="radiogroup" aria-label={t("selectLanguage")}>
