...
const DoctorSidebar: React.FC<DoctorSidebarProps> = ({ className }) => {
  ...
  return (
    <>
      ...
      {isSidebarOpen && (
        <div
          className="doctor-mobile-backdrop fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs md:hidden"
          onClick={() => dispatch(closeSidebar())}
        />
      )}
      <aside
        className={cn(
          "doctor-mobile-sidebar fixed top-0 bottom-0 left-0 z-50 w-[280px] sm:w-[320px] bg-[#9BACD8] flex flex-col justify-between md:hidden shadow-2xl transition-transform duration-300 text-slate-800 font-sans",
